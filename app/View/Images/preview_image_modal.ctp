  <div class="modal-header">
        ...
        <p class="modal-title" id="myModalLabel"><?php  echo $header; ?></p>      
  </div>
					...
					<div class="" style="margin: 0px auto; padding: 20px; color: grey;">
						...
						<?php
							echo $this->Form->create('Producto', array(
								'url' => array('controller' => 'images', 'action' => 'image_temp',$imagen_id,$tipo_imagen),
								'id' =>'cropImage',
								'class' => 'formblock',
								'style' => '',
								'type' => 'file',
							...
							<div class="form-group">										
									<p> 
									  Seleccione una imagen de <?php 
									  $dim = Configure::read('TIVIA_CONFIG.CROP_SIZE.'.$tipo_imagen);
									  echo 'min ' . $dim['MinWidth'].'x'.$dim['MinWidth'].' - max  '. $dim['MaxWidth'].'x'.$dim['MaxWidth']  ;?>									  
									  <span class="choose_file" style="margin-left: 10px; padding: 6px 5%;width: 25%;">
										...
										<?php 
										$arrsizeEncode =  json_encode($arrsize);																			
...
$('#guardaImagen').on('click', function(){
    ...
    var formUrlImgProduct = $('#cropImage').attr('action');                  
   $.ajax({
       type: 'POST',
    	 ...
    	 //si se carga la img se muestra en contenedores configurados en listing y modal          	
    	   if (dataimgcropped == 1){
               ...
               notifyInfo(msg);                
    			...
    			/* $('#hideFotoPeq').show(); */	   
    			$("#hideFotoPeq").css("display", "inline-block"); 
    			$("#hideFotoPeq").css("margin-left", "100px");
    			$('.choose_file_form').css("padding", "3px 5%"); 
    	    	$('#imagePreviewPeq').empty();
    	        $('#imagePreviewPeq').css("background-image", "url("+dataimgcropped+")");  	       	      
